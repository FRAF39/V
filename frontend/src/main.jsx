...
const api=async(u,o={})=>{
  ...
  catch(e){ throw Error("Network error: " + (e?.message||"unable to reach the server")); }
  const d=await r.json().catch(()=>({}));
  if(!r.ok){
    const detail=Array.isArray(d.details)?` — ${d.details.join("; ")}`:(d.details?` — ${String(d.details)}`:"");
    throw Error((d.error||d.message||`Request failed (${r.status})`)+detail);
  }
  return d;
};
...
function ProjectModal({p,onClose,onRefresh}){
 ...
 const del=async()=>{
  if(!confirm(`Delete project ${p.name}? All uploaded files and deployment records will be permanently removed.`))return;
  setBusy(true);
  try { await api(`/api/projects/${p.id}`,{method:"DELETE"}); onClose(); onRefresh(); }
