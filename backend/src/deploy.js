...
async function syncProjectToGitHub({userId,projectId,oldFiles}){
  ...
  await walk(root);
  for(const f of oldFiles||[]) entries.push({path:`${prefix}/${f.path}`,mode:"100644",type:"blob",sha:null});
...
async function renderServiceFor({id,projectId,runtime,name,buildCommand,startCommand,existingServiceId}){
  ...
  if(isStatic) body.serviceDetails={buildCommand:buildCommand||"echo 'static site ready'",publishPath:"."};
  else body.serviceDetails={runtime,plan:config.renderPlan,region:config.renderRegion,envSpecificDetails:{buildCommand:buildCommand||(runtime==="node"?"npm install":"pip install -r requirements.txt || true"),startCommand:startCommand||(runtime==="node"?"npm start":"gunicorn app:app --bind 0.0.0.0:$PORT")},healthCheckPath:"/"};
  const created=await api("https://api.render.com/v1/services",{method:"POST",body:JSON.stringify(body)}); return {serviceId:created.id||created.service?.id,url:created.url||created.service?.url};
}
